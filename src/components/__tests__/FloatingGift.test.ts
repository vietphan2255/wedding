...
import { resolveLeg } from '../FloatingGift'

const slot = (over = {}) => ({ image: '', size: 72, offset: 8, speed: 60, wait: 1.5, ...over })
...
describe('resolveLeg', () => {
  ...
  it('mirrors slot A for leg B (borrowing A settings) when only A has an image', () => {
    const fg = {
      enabled: true,
      slotA: slot({ image: 'a.png', size: 80, speed: 120 }),
      slotB: slot(),
    }
    expect(resolveLeg('A', fg)).toMatchObject({ src: 'a.png', flip: false })
    expect(resolveLeg('B', fg)).toMatchObject({ src: 'a.png', flip: true, size: 80, speed: 120 })
  })

  it('mirrors slot B for leg A (borrowing B settings) when only B has an image', () => {
    const fg = {
      enabled: true,
      slotA: slot(),
