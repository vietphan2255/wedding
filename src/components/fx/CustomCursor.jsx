...
// (optional `data-cursor-label` overrides the word). Self-disables on coarse
// pointers and reduced-motion, restoring the native cursor.
export default function CustomCursor() {
  const reduce = useReducedMotion()
  const [enabled, setEnabled] = useState(false)
  const [active, setActive] = useState(false) // hovering an opt-in element
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (target) {
        setActive(true)
        setLabel(target.getAttribute('data-cursor-label') || target.getAttribute('data-cursor') || '')
      } else {
        setActive(false)
        setLabel('')
      }
